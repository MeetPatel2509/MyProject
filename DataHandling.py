...
def cleanData():
    ...
    #full data
    sdf.write.option("header", "true").mode("overwrite").parquet(config.get('s3path')[0],compression="gzip")

    #partioned data
    sdf.write.option("header", "true").mode("overwrite").partitionBy("state_name","Years","Months").parquet(config.get('s3path2')[0],compression="gzip")
cleanData()